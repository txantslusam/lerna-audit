...
async function lernaAudit(_flags) {
  ...
  for (const lernaPackage of lernaPackages) {
    ...
    if (!flags.json) console.log(`Running ${lernaPackage.name}`);
    ...
    try {
      ...
      if (!flags.json) console.log(`Run audit in ${lernaPackage.location}`);
      ...
      if (!flags['no-report']) {
        ...
        generateReport(flags, auditResult.stdout.toString(), lernaPackage.name);
      }
